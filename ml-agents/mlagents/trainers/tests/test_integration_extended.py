        ...
        trainer_settings = TrainerSettings(
            ...
            reward_signals={
                RewardSignalType.EXTRINSIC: RewardSignalSettings(
                    gamma=0.99, strength=1.0
                ),
                RewardSignalType.CURIOSITY: RewardSignalSettings(
                    gamma=0.99, strength=0.01
                ),
                RewardSignalType.GAIL: RewardSignalSettings(gamma=0.99, strength=0.01),
            },
        )
...
class TestPhase2Integration:
    ...
    def test_torchscript_optimization_config(self):
        ...
        network_settings = NetworkSettings(
            enable_torchscript=True, torchscript_optimize_for_inference=True
        )
    ...
    def test_profiling_compatible_config(self):
        ...
        trainer_settings = TrainerSettings(
            trainer_type="ppo",
            hyperparameters=PPOSettings(
                batch_size=512, buffer_size=5120  # Smaller batch for profiling
            ),
            network_settings=NetworkSettings(
                hidden_units=128, num_layers=2  # Smaller network
            ),
            max_steps=100000,
