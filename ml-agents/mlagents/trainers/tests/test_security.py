...
class TestSubprocessSecurity:
    ...
    def test_no_shell_true_in_subprocess_calls(self):
        ...
        shell_true_pattern = re.compile(
            r"subprocess\.(call|run|check_call|Popen).*shell\s*=\s*True"
        )
    ...
    def test_no_eval_exec_in_production_code(self):
        ...
        production_files = [
            f
            for f in python_files
            if "test" not in str(f) and "_pb2" not in str(f) and "venv" not in str(f)
        ]
...
class TestCredentialSecurity:
    ...
    def test_no_hardcoded_api_keys(self):
        ...
        patterns = [
            ...
            re.compile(
                r'secret[_-]?key\s*=\s*["\'][a-zA-Z0-9]{20,}["\']', re.IGNORECASE
            ),
