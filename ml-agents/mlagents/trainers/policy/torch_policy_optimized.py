...
class TorchPolicyOptimized(TorchPolicy):
    ...
    def __init__(
        ...
        if self.enable_torchscript:
            logger.info(
                "TorchScript optimization will be enabled after first forward pass"
            )
    ...
    def evaluate(
        ...
        if not self._compilation_attempted and self.enable_torchscript:
            try:
                ...
                logger.warning(
                    f"Could not create example inputs for TorchScript compilation: {e}"
                )
    ...
    def _process_obs_for_compilation(self, decision_requests: DecisionSteps):
        ...
        try:
            ...
            tensor_obs = [
                ModelUtils.list_to_tensor(obs) for obs in decision_requests.obs
            ]
    ...
    def benchmark_inference_speed(self, num_iterations: int = 1000):
        ...
        logger.info(
            "Benchmark requires example inputs - implement based on your use case"
        )
