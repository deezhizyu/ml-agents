...
class TorchScriptOptimizer:
    ...
    def compile_model(
        ...
        try:
            ...
            failure_rate = _compilation_stats["failures"] / _compilation_stats["attempts"]
    ...
    def benchmark_model(
        ...
        logger.info("="*60)
        logger.info(f"Original model: {original_time:.3f}s ({results['original_fps']:.1f} inferences/sec)")
        logger.info(f"Optimized model: {optimized_time:.3f}s ({results['optimized_fps']:.1f} inferences/sec)")
