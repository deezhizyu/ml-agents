...
class CurriculumScheduler:
    ...
    def __init__(
        self, lessons: List[Lesson], parameter_name: str, min_lesson_steps: int = 10000
    ):
        """
    ...
    def get_current_parameters(self) -> Dict:
        ...
        return {self.parameter_name: self.current_lesson.value}
    ...
    def should_progress(
        ...
        if self.steps_in_current_lesson < self.min_lesson_steps:
            return (
                False,
                f"Only {self.steps_in_current_lesson}/{self.min_lesson_steps} steps",
            )
        ...
        should_advance, new_smoothing = criteria.need_increment(
            training_progress, reward_buffer, self.smoothed_reward
        )
...
class MultiParameterCurriculumScheduler:
    ...
    def add_scheduler(self, parameter_name: str, scheduler: CurriculumScheduler):
    ...
    def update_all(
        ...
        for param_name, scheduler in self.schedulers.items():
            should_advance, reason = scheduler.should_progress(
                reward_buffer, training_progress
            )
