...
try:
    ...
    print(
        f"Observation size: {np.prod(obs_shape) * obs_dtype(0).itemsize / 1024:.1f} KB"
    )
    ...
    try:
        ...
        print(
            f"  Read: {read_time/num_iterations*1000:.3f}ms per operation (zero-copy)"
        )
        print(
            f"  Total: {(write_time + read_time)/num_iterations*1000:.3f}ms per cycle"
        )
